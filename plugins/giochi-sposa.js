...
let handler = async (m, { conn, command }) => {
    try {
        if (command === 'sposa') {
            ...
            global.marriage_proposals[proposee] = {
                proposer,
                msgId: msg.key.id,
                chat: m.chat,
                timeout: setTimeout(async () => {
                    delete global.marriage_proposals[proposee];
...
handler.before = async (m, { conn }) => {
    if (!global.marriage_proposals || !global.marriage_proposals[m.sender]) return;

    const proposal = global.marriage_proposals[m.sender];
    ...
    if (buttonId) {
        if (buttonId === `accept_${m.sender}`) {
            ...
            delete global.marriage_proposals[m.sender];
